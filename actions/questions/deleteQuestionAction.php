...
//Vérifier si l'utilisateur est authentifié au niveau du forum
session_start();
...
if(isset($_GET['id']) AND !empty($_GET['id'])){
    ...
    $checkIfQuestionExists = $dbd->prepare('SELECT id_auteur FROM question WHERE id = ?');
    ...
    if($checkIfQuestionExists->rowCount() > 0){
        ...
        if($questionsInfos['id_auteur'] == $_SESSION['id']){
            ...
            $deleteThisQuestion = $dbd->prepare('DELETE FROM question WHERE id = ?');
